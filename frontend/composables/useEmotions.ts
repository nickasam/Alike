...
const EMOTIONS: EmotionMeta[] = [
  ...
  { key: 'break', label: '崩溃', bg: 'rgba(167,139,250,.16)', color: '#a78bfa', icon: ['M13 2 3 14h9l-1 8 10-12h-9l1-8z'] },
  { key: 'numb', label: '麻木', bg: 'rgba(100,116,139,.18)', color: '#94a3b8', icon: ['M5 12h14'] },
...
const byLabel: Record<string, EmotionMeta> = Object.fromEntries(
...
export function useEmotions() {
  ...
  /** 依据 key 或中文 label 查询（兼容日记 mood 历史上存 label 的数据）。 */
  function resolve(keyOrLabel?: string | null): EmotionMeta | undefined {
    if (!keyOrLabel) return undefined
    return byKey[keyOrLabel] ?? byLabel[keyOrLabel]
  }

  return { emotions: EMOTIONS, find, resolve }
}
